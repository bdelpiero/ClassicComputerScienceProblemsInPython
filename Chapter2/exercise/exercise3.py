...
# The solution uses dfs algorithm, which required implementing a __hash__ method for storing the visited states in a dictionary
# and returning the same state when the variables are equal even though the objects are referentially different.
# There is no solution for this algorithm when the MAX_NUM is greater than 3
if __name__ == "__main__":
    start: MCState = MCState(MAX_NUM, MAX_NUM, True)
    solution, _ = dfs(start, MCState.goal_test, MCState.successors)
    if solution is None:
